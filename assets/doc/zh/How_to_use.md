...
- **已知缺口**：MaaTools 协议只有触摸（`TUCH`）与截图指令，没有键盘/文本指令，依赖键盘的操作只能改用触摸兜底（战斗开始等已处理），其中**「使用编队码」无法模拟文本输入**：脚本输不进编队码，会记录 `编队码加载失败，继续使用当前队伍配置` 并按当前队伍配置继续执行（不会中断任务）。该功能使用频率低，暂不提供兜底；需要时请在游戏内手动保存/选择队伍，或改用上面的模拟器后台模式（ADB）。
