...
- **已知缺口**：MaaTools 协议只有触摸（`TUCH`）与截图指令，没有键盘/文本指令。依赖键盘的操作已改为触摸兜底：**开始战斗**优先点界面上的开始按钮确认当前选择（保留第一回合守备），点不动才退化为胜率自动选择 —— 后者会**覆盖手动守备/链接战划线**（日志会提示）。**镜牢战败重开**没有 ESC 通道时改点战斗内暂停按钮，打开与 ESC 相同的设置浮层；**返回主界面**的最后手段也从按 ESC 改为点界面上的设置/返回按钮；**镜牢键盘寻路**（`mirror_keyboard_navigation` 与简单键盘寻路）在触摸端自动回退为点击寻路（日志一次性提示）；**镜牢地图缩放**（PC 的滚轮）在触摸端改用双指捏合。其中**「使用编队码」无法模拟文本输入**：脚本输不进编队码，会记录 `编队码加载失败，继续使用当前队伍配置` 并按当前队伍配置继续执行（不会中断任务）。该功能使用频率低，暂不提供兜底；需要时请在游戏内手动保存/选择队伍，或改用上面的模拟器后台模式（ADB）。
